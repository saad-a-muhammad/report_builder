...
exports.createReport = catchAsyncErrors(async ({body:{ joins, connection }},res) => {
  ...
  let query = `SELECT * FROM ${p_table[0]} ${p_table[0].substring(0,2)} ${joins[0].type} 
                ${joins[0].to_table} ${joins[0].to_table.substring(0,2)} ON 
                    ${joins[0].from_table.substring(0,2)}.${joins[0].from_column.column_name} = ${joins[0].to_table.substring(0,2)}.${joins[0].to_column.column_name} 
               `;
  if (joins.length>0) {
